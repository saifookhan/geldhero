...
const getGoalColorClass = (goalType: string) => {
  switch (goalType.toLowerCase()) {
    ...
    case 'home-furnishing': return 'bg-purple-100 text-purple-800'
    case 'education': return 'bg-indigo-100 text-indigo-800'
    case 'emergency-fund': return 'bg-orange-100 text-orange-800'
    case 'car': return 'bg-gray-100 text-gray-800'
...
export function AdminDashboard() {
  ...
  const [filteredResponses, setFilteredResponses] = useState<CalculatorResponse[]>([])
  const [loading, setLoading] = useState(true)
  const [selectedResponse, setSelectedResponse] = useState<CalculatorResponse | null>(null)
  ...
  useEffect(() => {
    ...
    if (searchTerm) {
      filtered = filtered.filter(response => 
        ...
        response.employment_status.toLowerCase().includes(searchTerm.toLowerCase())
      )
    }

    if (filterGoalType) {
      filtered = filtered.filter(response => response.goal_type === filterGoalType)
    }

    if (filterRiskComfort) {
      filtered = filtered.filter(response => response.risk_comfort === filterRiskComfort)
    }
  ...
  const avgAge = responses.length > 0 
    ? Math.round(responses.reduce((sum, r) => sum + r.age, 0) / responses.length)
    : 0
  const mostCommonGoal = responses.length > 0
    ? responses.reduce((acc, r) => {
        acc[r.goal_type] = (acc[r.goal_type] || 0) + 1
        return acc
      }, {} as Record<string, number>)
    : {}
  const topGoal = Object.keys(mostCommonGoal).reduce((a, b) => 
    mostCommonGoal[a] > mostCommonGoal[b] ? a : b, ''
  )
        ...
        <div className="mb-8">
          <div className="flex items-center justify-between">
            <div>
              ...
              <p className="text-gray-600 mt-2">Manage calculator responses and view analytics</p>
            </div>
            <Button onClick={fetchResponses} disabled={loading} className="flex items-center gap-2">
        ...
        <div className="grid grid-cols-1 md:grid-cols-4 gap-4 mb-8">
          ...
          <div className="bg-white p-6 rounded-lg shadow-sm border">
            <div className="flex items-center">
              ...
              <div className="ml-4">
                <p className="text-sm font-medium text-gray-500">Most Common Goal</p>
